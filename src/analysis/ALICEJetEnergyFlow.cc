#include "ALICEJetEnergyFlow.h"
#include "TMath.h"

// using namespace Analysis;

// Register the module with the base class
RegisterAnalysisModule<ALICEJetEnergyFlow> ALICEJetEnergyFlow::reg("ALICEJetEnergyFlow");

ALICEJetEnergyFlow::ALICEJetEnergyFlow(std::string name_in) : name(name_in), ui(-123456)
{
...
ALICEJetEnergyFlow::~ALICEJetEnergyFlow()
{
  std::cout << "-$-Deleting " << name << std::endl;
}

int ALICEJetEnergyFlow::ReadOptionParametersFromXML()
{
...
void ALICEJetEnergyFlow::ShowParamsSetting()
{
  std::cout << "[AnalysisModuleBase] ***-------------------------------------------" << std::endl;
  std::cout << "[AnalysisModuleBase] *** [ALICEJetEnergyFlow]" << std::endl;
...
void ALICEJetEnergyFlow::OneEventAnalysis(std::vector<std::shared_ptr<Particle>> particle_list, int i_tag_particle)
{
      ...
      for (int ir = 0; ir < jetR.size() - 1; ir++)
      {
        ...
        for (std::size_t i_jet1 = 0; i_jet1 < jet1_list.size(); ++i_jet1)
        {
          ...
          if (matched_jet1_candidate_index == i_jet1)
          {
            ...
            //--------------------------------------------------
            // std::cout << "[ALICEJetEnergyFlow] Matched narrower cone " << r_cone1 << " jet: " << jet1.pt() << std::endl;
            // std::cout << "[ALICEJetEnergyFlow] Matched wider cone " << r_cone2 << " jet: " << matched_jet2_candidate.pt() << std::endl;
            // std::cout<<"====="<<std::endl;
            //--------------------------------------------------
            ...
            for (int iv = 0; iv < variables.size(); iv++)
            {
              ...
              if (variables[iv] == "deltaPt")
              {
                ...
                // std::cout << "[ALICEJetEnergyFlow] Filling histograms for variable" << iv << " : " << variables[iv] << std::endl;
              }
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
void ALICEJetEnergyFlow::CombineHist(int iv, int ir, int ijp, int ijr, int ipp, int ipr, int ip)
{
  //
  std::string hist_name = GetHistName(iv, ir, ijp, ijr, ipp, ipr, ip);
  std::cout << "[ALICEJetEnergyFlow] hist_name = " << hist_name << std::endl;
  ...
  if (nJetTotal != 0)
  {
    ...
    total_hist->Print("ALICEJetEnergyFlow_");
  }
  else
  {
    std::cout << "[ALICEJetEnergyFlow] 0-total Jet" << std::endl;
    std::cout << "[ALICEJetEnergyFlow] Skip. " << std::endl;
  }
  total_hist->DeleteTH();
}
