...
#############################################################################################################
def CombineMixedEvent(id_start, id_end, input):

  output_file = os.path.join(input, jf.MixedEventFileName() )
  print('Mixed Event File:', output_file)
  f_mixed_event = open(output_file,'w')

  for index in range(id_start, id_end):
    input_file = os.path.join(input, jf.MixedEventFileName(index) )
